...
create_ldref_sqlite <- function(bfile, dbname, tag_r2=0.6)
{
	...
	cmd <- paste0("plink --bfile ", bfile, " --keep-allele-order --exclude ", bfile, ".indels --r in-phase with-freqs gz --out ", bfile, " --ld-window-kb 250 --ld-window 1000 --ld-window-r2 ",  tag_r2)
	system(cmd)
	...
	write.table(cmd, file=paste0(bfile, ".ld.sqlite"), row=F, col=F, qu=F)
	unlink(dbname)
