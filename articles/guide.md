...
We developed a format for storing and harmonising GWAS summary data
known as [GWAS VCF
format](https://github.com/MRCIEU/gwas-vcf-specification). This format
is effective for being very fast when querying chromosome and position
ranges, handling multiallelic variants and indels.

All the data in the [IEU GWAS database](https://gwas.mrcieu.ac.uk/) is
available for download in the GWAS VCF format. This R package provides
fast and convenient functions for querying and creating GWAS summary
data in GWAS VCF format. The package builds on the
[VariantAnnotation](https://bioconductor.org/packages/release/bioc/html/VariantAnnotation.html)
Bioconductor package, which itself is based on the widely used
[SummarizedExperiment](https://bioconductor.org/packages/release/bioc/html/SummarizedExperiment.html)
Bioconductor package.

## External tools

For some VCF querying functions it is faster to optionally use
[bcftools](https://samtools.github.io/bcftools/bcftools.html), and when
available the R package will use that strategy. To set a location for
...
Note that there is bcftools binary for Windows available, so some
querying options will be slower on Windows.

For LD related functions the package uses [plink
1.90](https://www.cog-genomics.org/plink/1.9). You can specify the
...
Alternatively you can automatically use use the binaries bundled here:
...
For this vignette we will use the bundled binaries in
`genetics.binaRies`.
...
To read an entire dataset use the `readVcf` function. As an example
we’ll use the bundled data which is a small subset of the Speliotes et
al 2010 BMI GWAS.
...
Please refer to the `VariantAnnotation` package documentation for full
details about the `CollapsedVCF` object. A brief summary follows.
...
There are 92 rows and 1 column which means 92 SNPs and one GWAS. See the
header information:
...
See the names of the GWAS datasets (in this case just one, and it refers
...
In this case you can obtain information about this study through the
`ieugwasr` package e.g. `ieugwasr::gwasinfo("IEU-a-2")`.

There are a few components within the object:

- `header` which has the meta data describing the dataset, including the
  association result variables
...
the `rowRanges` object is a `GenomicRanges` class, which is useful for
...
The VCF object is somewhat complex and you can read more about it in the
...
it. For example, create a `GRanges` object which is great for fast
...
The direct conversion to formats for tools such as TwoSampleMR, coloc,
and many others can also be made using the
<https://github.com/mrcieu/gwasglue> R package.

## Reading in with filters

The
[`query_gwas()`](https://mrcieu.github.io/gwasvcf/reference/query_gwas.md)
function takes either a filename to a vcf file, or vcf object as the
main argument. You can then query on `rsid`, `pval` or `chrompos`. For
example
...
are each identical, but the former saves time and memory because it is
querying the file using an index and only reading in what is required.
...
It’s possible to have multiple GWAS studies per vcf. You can specify
...
Note that querying by chrompos is the fastest way to deal with VCFs, use
this over rsid where possible when speed is an issue.

## Indexing rsid values

Querying by rsid is slow. If a large number of queries by rsid are to be
performed then it could be worth generating an index which would speed
up the querying. This approach uses
[SQLite](https://www.sqlite.org/index.html) to create a local database,
linking rsid to chromosome and position. It strips out the ‘rs’ from the
rs identifiers to make fast searchers by integer. The concept is based
on that developed here:
[bioforensics/rsidx](https://github.com/bioforensics/rsidx).
...
Querying by p-value is slow. It could be worth generating an index file
for p-values to speed this up. Similar to rsid queries, it uses an
sqlite database linking -log10 pvalues to chromosome and position.
...
The fastest way to query VCFs is by specifying chromosome and position.
...
Querying by p-value or rsid is also possible but is slower as only
...
If a set of rsids are requested from a vcf but some are absent, a
...
- using a set of genotyped samples as an LD reference panel (e.g. 1000
  genomes data) - this is slow but relatively convenient
- compiling an LD tag list from an LD reference panel - once generated
  this is very fast

### Using an LD reference panel

An LD reference panel can be obtained from here:
<http://fileserve.mrcieu.ac.uk/ld/data_maf0.01_rs_ref.tgz>. This dataset
comprises Europeans from the 1000 genomes project, in plink format, and
including only SNPs with MAF \> 0.01, and with the reference alleles
aligned to the human genome reference sequence. For this vignette we can
...
We also need to provide a path to the plink binary used to generate LD
calculations. This can be done through the `genetics.binaRies` package
as with bcftools
...
The rs4442317 variant is not present in the vcf file, i.e. if we query
that variant:
...
You may also extract only the best available proxies even if the
requested rsids are present, by using `proxies="only"`. An example of
this shows that the effect size estimates for the proxy variants are
...
![Plot of the target data effects against the proxy data
effects](figure/target-effects-plot-1.png)

Plot the target data effects against the proxy data effects

### Compiling a list of tagging variants

Using the LD reference panel described above, it is possible to create a
sqlite tag reference panel using the following commands. First get an
...
If you have GWAS summary data in a text file or data frame, this can be
...
You may want to first harmonise the data so that all the non-effect
alleles are aligned to the human genome reference. See the
[gwasglue](https://github.com/MRCIEU/gwasglue) package on some functions
to do this.

## Creating a gwasglue2 SummarySet object from a vcf file

Although still under development, if compared with its predecessor, the
[gwasglue2](https://mrcieu.github.io/gwasglue2/) package has several new
features, including the use of S4 R objects.

It is possible to create a `SummarySet` object from a GWAS-VCF file or
VCF object e.g. output from
...
Once the `SummarySet` objects are created, it is possible to use
`gwasglue2` to harmonise data, harmonise against a LD matrix, remap
genomic coordinates to a different genome assembly, convert to other
formats and more.
